...
def parse_args():
    parser = argparse.ArgumentParser(description="Main 3D everything train/test file")
    parser.add_argument(
        "--gpu",
        dest="gpu_id",
    ...
    parser.add_argument(
        "--net", dest="net_name", help="name of the net", default=None, type=str
    )
    parser.add_argument(
        "--model",
        dest="model_name",
    ...
    parser.add_argument(
        "--exp", dest="exp", help="name of the experiment", default=None, type=str
    )
    parser.add_argument(
        "--weights",
        dest="weights",
...
def main():
    ...
    if args.model_name is not None:
        cfg_from_list(["CONST.NETWORK", args.model_name])
    if args.dataset is not None:
        cfg_from_list(["DATASET", args.dataset])
    if args.weights is not None:
        cfg_from_list(["WEIGHTS", args.weights])
